	...
	function __construct() {
		...
		if(!empty($_POST)) {
               if(!empty($_POST['email'])){
				  ...
				  $statement = $DBH->prepare("select count(*) as count from  detox_order where email='$email' AND detox_type='21Day'");
				  $statement->execute(array(':name' => $email));
                  $row = $statement->fetch(PDO::FETCH_ASSOC);		 	
                  $count=$row['count'];  			   
					  if($count<1){						
					   
					   $this->error = '<div class="alert alert-error">'._('You must have a current subscription to register for the 7 Day Detox. <a href="http://divinehealthdetox.com/shop/home.php">Subscribe Now</a>').'</div>';
					   }
					  } 
