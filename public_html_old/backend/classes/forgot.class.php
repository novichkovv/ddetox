	...
	public function modal_process() {
		...
		if(isset($_POST['usernamemail'])) {
			...
			if (empty($usernamemail)) {
				...
				$stmt   = parent::query("SELECT * FROM `login_users` WHERE `username` = :usernameEmail OR `email` = :usernameEmail;", $params);
				 ...
				 if( $stmt->rowCount() > 0 ) {
					...
					$shortcodes = array(
						...
						'reset'         =>  SITE_PATH . "backend/forgot.php?key=$key"
					);
				...
				} else { $this->error = '<div class="alert alert-error">'._('Hello, We searched for you high and low but couldn\'t find you :(').'</div>'; }
			}
