...
//Added function to search via email address (Paul B. 11/5/2013)
		$params = array( ':searchQ' => $_POST['searchUsers'] . '%' );
		$sql = "SELECT distinct username as suggest, user_id, email
				FROM login_users
		...
		while($suggest = $stmt->fetch(PDO::FETCH_ASSOC))
			echo "<p><a href='users.php?uid=" . $suggest['user_id'] . "'>" . $suggest['suggest'] . " - " .$suggest['email'] ."</a></p>\n";
